...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    label = label[0,:,:,:]
    label = intensity_to_label(label)
    print(image.shape)
    print(label.shape)
    if len(image.shape) == 4:
        prediction = np.zeros_like(label, dtype=np.int)
        for ind in range(image.shape[1]):
            slice = image[:, ind, :, :]
            x, y = slice.shape[1], slice.shape[2]
            if x != input_size[0] or y != input_size[1]:
                slice = zoom(slice, (1, input_size[0] / x, input_size[1] / y), order=3)
            new_x, new_y = slice.shape[1], slice.shape[2]
            if new_x != patch_size[0] or new_y != patch_size[1]:
                slice = zoom(slice, (1, patch_size[0] / new_x, patch_size[1] / new_y), order=3)
