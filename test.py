...
class_to_name = {1: 'kidney', 2: 'tumor', 3: 'cyst'}
...
def inference(args, multimask_output, db_config, model, test_save_path=None):
    db_test = db_config['Dataset'](base_dir=args.volume_path, list_dir=args.list_dir, split='valid_vol')
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
    logging.info(f'{len(testloader)} test iterations per epoch')
    model.eval()
    metric_list = 0.0
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        b, c, d, h, w = sampled_batch['image'].shape
...
if __name__ == '__main__':
    ...
    parser.add_argument('--volume_path', type=str, default='./data/valid_vol/')
    parser.add_argument('--dataset', type=str, default='KiTS23', help='Experiment name')
    parser.add_argument('--num_classes', type=int, default=3)
    parser.add_argument('--list_dir', type=str, default='./lists/lists_KiTS23/', help='list_dir')
    parser.add_argument('--output_dir', type=str, default='./KiTS23/output')
    ...
    parser.add_argument('--ckpt', type=str, default='./checkpoint/sam_vit_h_4b8939.pth',
                        help='Pretrained checkpoint')
    parser.add_argument('--lora_ckpt', type=str, default='./KiTS23/output/KiTS23_512_pretrain_vit_h_60k_epo600_bs26_lr0.0005_s42/epoch_20.pth', help='The checkpoint from LoRA')
    ...
    sam, img_embedding_size = sam_model_registry[args.vit_name](image_size=args.img_size,
                                                                    num_classes=args.num_classes,
                                                                    checkpoint=args.ckpt, pixel_mean=[16.9, 16.9, 16.9],
                                                                    pixel_std=[43.5, 43.5, 43.5])
